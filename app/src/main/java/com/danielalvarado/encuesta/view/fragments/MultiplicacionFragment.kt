...
import com.danielalvarado.encuesta.databinding.FragmentMultiplicacionBinding
...
class MultiplicacionFragment : Fragment() {
    private lateinit var binding: FragmentMultiplicacionBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentMultiplicacionBinding.inflate(inflater, container, false)
        cargarSpinner()
        buttonConfirm()
    ...
    private fun obtenerResultados(){
        if (binding.actSuma1.text.toString().toInt() == 30){
            resultado1 += 5
        }
        if (binding.actSuma2.text.toString().toInt() == 20){
            resultado1 += 5
        }
        ...
        if (binding.actSuma5.text.toString().toInt() == 40){
            resultado2 += 5
        }
        res = Resultados(
            obtenerFecha(),
